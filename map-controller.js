...
class MapController {
    ...
    initMap() {
        ...
        this.map = L.map('map-container').setView([this.currentLocation.lat, this.currentLocation.lng], 13);
...
addGabesLandmarks() {
    ...
    } location - Location object with lat and lng
     * @param {number}
    ...
    } confidenceScore - Confidence score from 0 to 100
     */
    updateLocationByConfidence(location, confidenceScore) {
        // Calculate radius based on confidence (lower confidence = larger radius)
        const radius = this.calculateRadiusByConfidence(confidenceScore);
